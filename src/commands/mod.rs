...
pub fn execute(cmd: NormalCommand, shared: &Arc<Shared>) -> RespValue {
    match cmd {
        ...
        NormalCommand::Incr { key } => strings::incrby(shared, key, 1),
        NormalCommand::Decr { key } => strings::decrby(shared, key, 1),
        ...
        NormalCommand::DecrBy { key, value } => strings::decrby(shared, key, value),
