...
async fn set_and_get() {
    ...
    assert_eq!(common::read_reply(&mut r).await, "+OK\r\n");

    w.write_all(b"GET foo\n").await.unwrap();
    assert_eq!(common::read_reply(&mut r).await, "$3\r\nbar\r\n");
}
...
async fn get_missing_key_returns_nil() {
    ...
    assert_eq!(common::read_reply(&mut r).await, "$-1\r\n");
}
...
async fn set_overwrites_existing_value() {
    ...
    w.write_all(b"GET k\n").await.unwrap();
    assert_eq!(common::read_reply(&mut r).await, "$2\r\nv2\r\n");
}
...
async fn del_missing_key_returns_zero() {
    ...
    assert_eq!(common::read_reply(&mut r).await, ":0\r\n");
}
